...
def update_and_write_to_online(user_id, asin, rating, timestamp):
    logger.debug(f"[update_and_write_to_online] user_id={user_id}, asin={asin}, rating={rating}, timestamp={timestamp}")
    ...
    old_asins = user_feature.get("user_rating_list_10_recent_asin", [""])[0].split(",") if user_feature.get("user_rating_list_10_recent_asin", [""])[0] else []
    old_timestamps = user_feature.get("user_rating_list_10_recent_asin_timestamp", [""])[0].split(",") if user_feature.get("user_rating_list_10_recent_asin_timestamp", [""])[0] else []
    ...
    current_ts = item_sequence_ts[-1] if item_sequence_ts and item_sequence_ts[-1] != -1 else int(datetime.now(timezone.utc).timestamp())
...
def lambda_handler(event, context):
    logger.info(f"Received event: {json.dumps(event)[:1000]}")  # Log tối đa 1000 ký tự, tránh log quá dài
