...
logger.info("S3 Client Endpoint: %s", boto_session.client("s3", endpoint_url=MINIO_EP).meta.endpoint_url)
...
def load_model_and_artifacts(model_name: str):
    ...
    try:
        pipeline_path = mlflow.artifacts.download_artifacts(run_id=run_id, artifact_path="item_metadata_pipeline/item_metadata_pipeline.pkl")
...
def export_to_onnx(model: torch.nn.Module, seq_len: int, feature_size: int) -> str:
    ...
    try:
        torch.onnx.export(
            ...
            input_names=[
                "user_ids", "input_seq", "input_seq_ts_bucket",
