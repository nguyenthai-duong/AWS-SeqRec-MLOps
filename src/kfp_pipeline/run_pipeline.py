...
def feature_engineering_op(
    ...
    try:
        ...
        subprocess.run(["uv", "run", pipeline_path], check=True)
...
feature_component = func_to_container_op(
    ...
    base_image="nthaiduong83/feature_pipeline:v6",
)
...
def feature_pipeline():
    ...
    feature_task = feature_component(
        output_path="/data/papermill-output/feature-output.ipynb"
    ).add_pvolumes({
        "/data": pvc
    }).add_env_variable(
        V1EnvVar(
        ...
        V1EnvVar(
            ...
            value_from=V1EnvVarSource(
                secret_key_ref=V1SecretKeySelector(
                    name="aws-credentials",
                    key="POSTGRES_URI_REGISTRY"
                )
            ),
        )
    ).add_pod_annotation(
        "debug/mount-path", "/data"
    ).set_memory_request("2Gi")

    feature_task.execution_options.caching_strategy.max_cache_staleness = "P0D"


    negative_sampling_task = negative_sampling_component(
        ...
        "debug/mount-path", "/data"
    ).set_memory_request("2Gi").after(feature_task)

    negative_sampling_task.execution_options.caching_strategy.max_cache_staleness = "P0D"

    prep_item2vec_task = prep_item2vec_component(
