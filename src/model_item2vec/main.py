...
def train_func(config, is_final_training=False):
    ...
    try:
        ...
        experiment_name = f"{base_experiment}/final_model" if is_final_training else f"{base_experiment}/hyperparameter_tuning"
        ...
        run_name = f"Final Model - SkipGram" if is_final_training else "Hyperparameter Tuning"
        mlflow_run = mlflow.start_run(run_name=run_name, tags=tags)
        
        params_to_log = {}
        for k, v in config.items():
            if k in ["max_epochs", "batch_size", "num_negative_samples", "window_size", "num_workers", "learning_rate", "l2_reg", "embedding_dim"]:
    ...
    try:
        ...
        lit_model = LitSkipGram(
            ...
            log_dir=os.path.join(config["checkpoint_dir"], f"trial_{train.get_context().get_trial_id()}" if not is_final_training else "final_model"),
        )
        ...
        trainer = L.Trainer(
            ...
            callbacks=[
                L.pytorch.callbacks.ModelCheckpoint(
                    dirpath=os.path.join(config["checkpoint_dir"], f"trial_{train.get_context().get_trial_id()}" if not is_final_training else "final_model"),
        ...
        if mlflow_run:
            try:
                ...
                if is_final_training:
                    metrics_to_log = {f"final.{k}": v for k, v in metrics_to_log.items()}
                else:
                    metrics_to_log = {f"tuning.{k}": v for k, v in metrics_to_log.items()}
                ...
                if is_final_training:
                    try:
                        ...
                        print(f"IDMapper saved and logged as artifact at {idm_temp_path}")
                        ...
                        for v in client.search_model_versions(f"name='{config['run_name']}_skipgram'"):
                            ...
                            try:
                                ...
                                other_val_loss = run_data.metrics.get("final.val_loss") or run_data.metrics.get("tuning.val_loss")
                                ...
                                print(f"Warning: Cannot load run data for version {v.version}: {str(e)}")
                        ...
                        if is_champion:
                            ...
                            print(f"Model version {this_version.version} is now CHAMPION (val_loss = {this_val_loss:.4f})")
                            for v in worse_versions:
                                try:
                                    ...
                                    print(f"Removed champion tag from version {v.version}")
                                except Exception as e:
                                    print(f"Warning: Failed to remove champion tag from version {v.version}: {str(e)}")
                        else:
                            print(f"Model version {this_version.version} is NOT champion (val_loss = {this_val_loss:.4f})")
